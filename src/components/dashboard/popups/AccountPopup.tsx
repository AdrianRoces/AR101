// src/components/dashboard/popups/AccountPopup.tsx
import { useState } from 'react';
import ConfirmationModal from '../../ui/ConfirmationModals';
import type { ConfirmationType, ActionType } from '../../ui/ConfirmationModals';
...
export default function AccountPopup({ isOpen, onClose, userData, onUserDataUpdate }: AccountPopupProps) {
  ...
  const [confirmationAction, setConfirmationAction] = useState<ActionType>('update');
  ...
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    if (newEmail && !/\S+@\S+\.\S+/.test(newEmail)) {
      showConfirmationMessage('error', 'update');
      return;
    }
    ...
    if (isSuccess) {
      ...
      if (newEmail && newEmail !== currentEmail) {
        ...
        showConfirmationMessage('info', 'update');
      } else {
        setCurrentEmail(updatedUserData.email);
        showConfirmationMessage('success', 'update');
      }
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        autoCloseDelay={confirmationType === 'success' ? 3000 : 5000}
      />
    </>
  );
}